/**
 * modules/ui.js — 所有畫面渲染與互動(最大檔)。
 * 結構:鎖定畫面 → 主畫面(App 網格)→ 各 App 頁面(聊天/社群/正文/角色與玩家/設定)。
 * 右側為預設收合的「管理輔助面板」(記憶管理與開發資訊),不屬於手機本體。
 *
 * ── 分區地圖(grep 「===」跳區;行號會漂,關鍵字錨點不會)──
 *   小工具            esc 在 utils/esc.js、avatarHtml、appHeader、bindBack
 *   初始化            applyTheme / initUI / renderAll
 *   手機頁面路由      renderPhone → 依 view 分派各 render*
 *   A. 鎖定畫面       renderLockScreen
 *   B. 主畫面         renderHome(App 網格)
 *   C. 聊天 App       friendRowsHtml / peekRowsHtml / groupRowsHtml
 *   D. 對話詳情       renderRoomView / renderMessages(DM·群聊·正文共用,最長)
 *   E. 社群 App       renderSocialFeed / renderSocialPost / 樓中樓
 *   F. 正文 App       renderStoryList
 *   G. 角色與玩家     renderPeople / renderCharacterDetail / renderPlayer
 *   設定 / AI 連線    apiSectionHtml / renderSettings
 *   世界書 / 日記     renderWorldbook* / renderCharacterDiary
 *   管理輔助面板      renderMemoryPanel / renderDevPanel(儀表板)
 *   Modal            openModal 基座 + 各 open*Modal(依「── Modal:」子標找)
 *   角色手機 App      renderCharPhone* / peekCardHtml(K 案收納頁)
 *
 * 維護紀律:改任何 prompt/隱私相關前先讀 dev-notes/HANDOVER.md 的隱私鐵律與事故史。
 */
...
import {
  ...
  deletePost, editPost, ensureSeedPosts, generateSocialReplies, groupComments,
...
/* ===================== 小工具(esc 在 utils/esc.js) ===================== */

// esc() 自 v91 起唯一實作在 utils/esc.js(三胞胎收斂,防不對稱防線)
...
function avatarHtml(authorId, cls = '') {
  if (authorId === 'player') {
    ...
    if (p.avatarImage) {
      return `<span class="avatar ${cls} img" aria-hidden="true"><img src="${esc(p.avatarImage)}" alt=""></span>`;
    }
  ...
  if (c.avatarImage) {
    return `<span class="avatar ${cls} img" style="--c:${esc(c.themeColor)}" aria-hidden="true"><img src="${esc(c.avatarImage)}" alt=""></span>`;
  }
...
function personaAvatarHtml(persona, cls = '') {
  ...
  if (persona.avatarImage) {
    return `<span class="avatar ${cls} img" aria-hidden="true"><img src="${esc(persona.avatarImage)}" alt=""></span>`;
  }
...
/** v94(t1):全站燈箱——點任何內容圖(貼文/訊息/相簿/分享卡)開黑底滿版,點任意處關。
 *  委派單一監聽,零 markup 變更;stopPropagation 防觸發氣泡的操作列切換。CSP 相容(無行內)。 */
function openLightbox(src) {
  let box = document.getElementById('lightbox');
  if (!box) {
    box = document.createElement('div');
    box.id = 'lightbox';
    box.innerHTML = '<img alt="大圖檢視">';
...
export function initUI() {
  document.addEventListener('click', (e) => {
    const img = e.target.closest?.('img.post-image, img.msg-image, img.shared-post-img, img.album-full');
    if (!img || !img.src) return;
    e.stopPropagation();
    openLightbox(img.src);
  }, true); // capture:搶在氣泡點擊切換操作列之前
...
function renderPhone() {
  ...
  switch (view) {
    ...
    case 'manual': renderManual(); break; // v101:玩家手冊
...
/* ===================== D. 對話詳情(renderRoomView / renderMessages;DM·群聊·正文共用) ===================== */
  ...
  messagesSinceSummary(room.id).length >= 30 ? '<span class="unread-dot" aria-label="建議摘要"></span>' : ''
    ...
    ${room.type === 'story' ? `
      <button class="status-bar-card ${room.statusBar?.trim() ? '' : 'empty'}" id="btnStatusBar" aria-label="編輯場景狀態">
        ${room.statusBar?.trim()
    ? `📍 ${esc(room.statusBar.trim().slice(0, 12))}${room.statusBar.trim().length > 12 ? '…' : ''}<span class="sb-caret">▾</span>`
    : '⊕ 設定場景狀態(時間/地點/衣著…)'}
      </button>` : ''}
    ${room.type === 'peek' ? `
...
function renderMessages() {
  ...
  const html = msgs.map((m, i) => {
    ...
    const imgHtml = m.image ? `<img class="msg-image" src="${esc(m.image)}" alt="訊息圖片">` : '';
    const shareHtml = m.sharedPost ? `
      <div class="shared-post-card">
        <div class="shared-post-head">${esc(m.sharedPost.authorName)} 的貼文</div>
        ${m.sharedPost.image ? `<img class="shared-post-img" src="${esc(m.sharedPost.image)}" alt="">` : ''}
...
function postCardHtml(p, { clickable = true } = {}) {
  ...
  return `
    <article class="post-card ${clickable ? 'clickable' : ''}" ${clickable ? `data-open-post="${esc(p.id)}"` : ''}>
      ...
      ${p.image ? `<img class="post-image" src="${esc(p.image)}" alt="貼文圖片">` : ''}
...
function renderSocialPost() {
  ...
  // v79(a案):歸樓改用 social.js 共用實作——舊版 hops<20 保險絲讓深樓留言無聲消失
  // (根因、防循環與孤兒平鋪防呆見 groupComments 註解)。
  ...
  // v79(b案):長樓折疊——同樓樓中樓超過 6 則,預設只顯示最新 6,前面收進展開鈕;
  // 行內回覆框若長在被收合的留言上,該樓自動展開(不然回覆框會憑空消失)。
  const FOLD_AT = 6;
  const commentHtml = roots.map((root) => {
    const kids = childrenByRoot.get(root.id) || [];
    const mustExpand = expandedThreads.has(root.id) || kids.slice(0, -FOLD_AT).some((k) => k.id === replyTargetId);
    const hiddenCount = (!mustExpand && kids.length > FOLD_AT) ? kids.length - FOLD_AT : 0;
    const shown = hiddenCount ? kids.slice(-FOLD_AT) : kids;
    return `
    <div class="comment-thread">
      ${oneComment(root)}
      ${hiddenCount ? `<button class="mini-btn thread-expand" data-expand-thread="${esc(root.id)}">▾ 展開先前 ${hiddenCount} 則留言</button>` : ''}
      ${shown.map((child) => `<div class="comment-child">${oneComment(child)}</div>`).join('')}
    </div>`;
  }).join('');
  ...
  els.phoneScreen.querySelectorAll('[data-expand-thread]').forEach((btn) => {
    btn.addEventListener('click', () => { expandedThreads.add(btn.dataset.expandThread); renderSocialPost(); });
  });
...
async function runMockSocialReplies(post, triggerText, triggerPersonaId = null, replyToName = null, threadReplyTo = null, banter = false) {
  const result = await generateSocialReplies({
    post, triggerText, triggerPersonaId, replyToName, replyToCommentId: threadReplyTo?.commentId || null, banter,
  }); // v79(c案):指名回覆帶 commentId → prompt 端帶那一樓完整串
  if (!result.ok) {
...
function openPhotoViewer(photoId) {
  ...
  openModal(`
    ...
    <img class="album-full" src="${esc(p.image)}" alt="回憶照片">
...
function openPhotoModal(photo, image) {
  ...
  openModal(`
    ...
    <img class="album-full" src="${esc(image)}" alt="回憶照片">
    <form id="photoForm">
...
function openSharePhotoModal(photo) {
  ...
  openModal(`
    <h3>分享回憶照片</h3>
    <img class="album-full" src="${esc(photo.image)}" alt="">
...
/* ===================== 設定頁:AI 連線區塊(apiSectionHtml / readApiForm) ===================== */
  ...
  return `
    <div class="api-section">
      ...
      <label class="field">上下文預算(字；對話歷史由新到舊裝進 prompt,裝滿即止——正文自動少帶幾則、短訊自動多帶)
        ...
        <span class="panel-note" id="budgetTokenHint"></span>
      </label>
      <label class="field">指令語言(v78 實驗:prompt 骨架語言;台詞/備註/世界書等內容不受影響,輸出仍強制繁中)
        <select id="apiPromptLang" class="theme-select" style="width:100%; margin-top:5px">
          <option value="zh" ${cfg.promptLang !== 'en' ? 'selected' : ''}>中文(預設)</option>
          <option value="en" ${cfg.promptLang === 'en' ? 'selected' : ''}>English(省指令 token,格式服從度較好)</option>
        </select>
      </label>
...
function readApiForm() {
  ...
  return {
    ...
    promptLang: g('#apiPromptLang') ? g('#apiPromptLang').value : 'zh',
...
function bindApiSection() {
  ...
  // v77(item 五):預算單位統一為「字」,輸入框旁即時顯示 token 換算(中文粗估 ×1.5)
  const budgetInput = els.phoneScreen.querySelector('#apiBudget');
  const budgetHint = els.phoneScreen.querySelector('#budgetTokenHint');
  const updateBudgetHint = () => {
    if (!budgetInput || !budgetHint) return;
    const n = Number(budgetInput.value) || 0;
    budgetHint.textContent = `≈ ${Math.round(n * 1.5).toLocaleString()} tokens(中文粗估 ×1.5)`;
  };
  if (budgetInput) { budgetInput.addEventListener('input', updateBudgetHint); updateBudgetHint(); }
...
function openCardPreviewModal(card) {
  ...
  openModal(`
    ...
    <div class="list-row" style="pointer-events:none">
      ${card.avatarImage ? `<span class="avatar img"><img src="${esc(card.avatarImage)}" alt=""></span>` : `<span class="avatar" style="--c:${esc(card.themeColor)}">${esc(card.avatarEmoji || card.name.slice(0, 1))}</span>`}
      <span class="list-main">
    ...
    return `
          <div class="icon-slot">
            <button class="icon-slot-preview ${cur ? 'has-img' : ''}" data-icon-up="${esc(app.id)}" aria-label="上傳 ${esc(app.label)} 圖示">
              ${cur ? `<img src="${esc(cur)}" alt="">` : `<span>${app.glyph}</span>`}
            </button>
...
/** v101:玩家手冊——按「你想幹嘛」查,每條三行內,原理不解釋(原理在 dev-notes)。
 * 維護規矩:每單交貨,新功能與新踩的坑同步補進來(尤其 QA 章),擁有者失憶就靠這本。 */
function renderManual() {
  const q = (t2, body) => `<details class="mem-group"><summary class="mem-subheading">${t2}</summary><div class="panel-note">${body}</div></details>`;
  const h2 = (t2) => `<div class="mem-heading">${t2}</div>`;
  els.phoneScreen.innerHTML = `
    ${appHeader('使用手冊')}
    <div class="phone-list">
    <div class="panel-note">這本按「你想做什麼」查,每條講三件事:在哪按、會發生什麼、要注意什麼。原理一律不解釋。遇到這裡沒寫的,直接問下一個對話的 AI,並叫它把答案補進這本。</div>

    ${h2('對話玩法')}
    ${q('跟一個角色私訊', 'DM 就是私訊。他知道什麼、不知道什麼,查記憶 App 裡的「誰知道什麼」對照表。')}
    ...
    ${h2('記憶(他會不會忘記事情)')}
    ${q('讓他永遠記得某件事', '把那條記憶按 📌 釘選——釘選的永遠隨身帶、沒有上限,這是唯一的鐵保證。重要的事都去釘。')}
    ${q('把一句話變成記憶', '訊息或貼文上按「記住」,存之前可以改寫。DM 記的只有他本人知道。')}
    ${q('管理全部記憶', '主畫面 ✦記憶:所有記憶依歸屬分組,頂端有「誰知道什麼」對照表。')}
    ${q('讓 AI 幫你挑值得記的', '設定開「記憶提案收件匣」(預設關,每段多一次小額呼叫)。聊一段後它提案,你在記憶頁逐筆採納/駁回;不採納就永遠不生效。')}
    ${q('「上限」到底是什麼意思', '所有上限都只是「這一發隨身帶多少」:歷史帶最近約 2 萬字、未釘記憶共帶 3000 字。<b>任何東西都不會被刪</b>,舊的都在手機裡,只是那一發沒帶在身上。')}
    ${q('長篇正文他忘了前情', '章節封存:整章收起來、前情摘要自動變場景記憶,下一章的說書人一定記得。跑一百章也不會忘第一章。')}

    ${h2('導演工具')}
    ${q('給這本戲下指令(節奏/尺度/狀態欄)', '房間設定裡的「作者備註」=最高優先導演指令,凌駕角色卡。狀態欄範本:<br>每則回覆開頭固定加入狀態欄(三行,之後才開始敘事):<br>📅:&lt;月&gt;月&lt;日&gt;日|HH:MM|星期&lt;X&gt;<br>🔮:&lt;所在地&gt;|&lt;具體地點&gt;<br>👗:&lt;穿著,只寫衣著,簡短&gt;')}
    ${q('標記你們進展到哪了', '房間設定的「關係階段」:寫「曖昧中」「冷戰中」之類,他的態度會跟著走。')}
    ${q('這本戲用貴模型、其他照舊', '房間設定的「這個房的模型」下拉。只影響主線回覆;心聲/偷看/日記照走便宜通道,不會跟著變貴。')}
    ${q('古代/架空角色不要滑手機', '角色卡勾 noPhone:不進社群、不用 Emoji、不知道現實時間。古裝戲必勾。')}

    ${h2('錢包')}
    ${q('看花了多少', '管理面板 → 開發資訊:今天/7日 token、最燒的對象。粗換算:正文一則約 1~2 元。')}
    ${q('省錢三旋鈕(由免錢到有感)', '① thinkingBudget 設 0(思考常常是白燒);② 上下文預算調低(歷史變短=每發便宜,但他近期記憶變短);③ 每房模型(貴的只給值得的房)。')}
    ${q('快取命中率是什麼', '同一房連續聊,重複的部分打 2.5~7.5 折。第一輪 0% 正常;同房越聊越高。這數字突然崩到很低=有東西壞了,回報。')}
    ${q('看到「單次請求過大」', '保險絲跳了=有東西壞了(正常一輩子不該看到)。把這句話原文回報給 AI。')}

    ${h2('備份(重要!)')}
    ${q('資料存在哪', '<b>只在這台裝置的瀏覽器裡</b>。清瀏覽器資料=全沒。看到「幾天沒備份」的橫幅就點它匯出。')}
    ${q('備份單獨一本戲', '房間的 ⋯ 選單 → 匯出。想大改前(拆卡/實驗)先匯出當保命符。')}
    ${q('匯入會不會蓋掉東西', '不會。匯入永遠建立新副本,絕不覆蓋既有的。')}

    ${h2('疑難雜症(你踩過的坑都在這)')}
    ${q('心聲/摘要失敗:「被思考吃光」', 'API 設定 → thinkingBudget 設 0。一勞永逸。')}
    ${q('失敗:「被供應商的安全過濾攔下」', '過濾有隨機性。API 設定調寬「內容安全等級」;還不行就把最近一句改寫再送。重按有時也會過。')}
...
/* ===================== Modal(openModal 基座 + 各 open*Modal) ===================== */
  ...
  return `
    ...
    <div class="field">頭像圖片
      <div class="avatar-upload">
        <span class="avatar-preview" id="avatarPreview">${c.avatarImage ? `<img src="${esc(c.avatarImage)}" alt="">` : '<span>無</span>'}</span>
...
function openPersonaModal(persona = null) {
  ...
  openModal(`
    ...
    <form id="personaForm">
      <div class="field">頭像圖片
        <div class="avatar-upload">
          <span class="avatar-preview" id="avatarPreview">${persona?.avatarImage ? `<img src="${esc(persona.avatarImage)}" alt="">` : '<span>無</span>'}</span>
          <input type="file" id="avatarFile" accept="image/*" hidden>
          <button type="button" class="mini-btn" id="btnAvatarPick">上傳(自動壓縮)</button>
          <button type="button" class="mini-btn" id="btnAvatarClear">移除</button>
        </div>
    ...
    onOpen(root) {
      ...
      fileInput.addEventListener('change', async () => {
        ...
        try {
          ...
          root.querySelector('#postImgPreview').innerHTML = `<img src="${esc(postImage)}" alt="">`;
...
/* ── Modal:通用確認(esc 過 title/body,見 XSS 防線) ── */
...
function openSharePostModal(post) {
  ...
  openModal(`
    ...
    <div class="shared-post-card preview">
      ...
      ${post.image ? `<img class="shared-post-img" src="${esc(post.image)}" alt="">` : ''}
