  ...
  const system = [
    ...
    (() => {
      ...
      const anchor = targetMsg?.role === 'character'
        ...
        : `請寫出紀錄最後那一刻`;
      return `【任務】以下是${isStory ? '一段正文場景' : isPeek ? '一段你們自己私下群組的聊天(玩家不在場)' : '一段群組聊天'}的紀錄(「旁白」是場景敘述)。${anchor},你(${character.name})心裡真正的想法——這一幕底下你沒說出口的部分(動作洩漏的、語氣藏著的、不敢講的)。這段心聲必須是「${character.name}」本人的第一人稱——絕對不要寫成其他成員的內心,不要替任何其他人發聲。純內心獨白,100~200 字${isStory ? ',以劇情當下的時空為準，不要提及現實日期' : ''}。不要對任何人喊話、不要引號包裹、不要描述自己的動作、不要任何標記格式。輸出繁體中文。最後再次確認:整段從頭到尾都必須是「${character.name}」本人的第一人稱內心——絕不可寫成旁白視角,絕不可描寫或代入任何其他人。`; // v101.1:身分鐵條複述收尾(v80 近輸出服從度;擁有者實案:三人正文戳楊皓心聲寫成謝子勳的特寫)
    })(),
  ].join('\n\n');
...
export function buildPhonePeekPrompt({ character, peekType }) {
  ...
  const TASKS = {
    ...
    playlist: `【任務】輸出你(${character.name})音樂 App 的「最近播放」5~6 首。每行一首，格式：歌名 — 歌手。優先使用真實存在的歌曲與歌手(只列歌名與歌手，絕不要輸出任何歌詞)，選歌要符合你的品味與此刻心境；想不到合適的真歌時，可以混入虛構的，但要聽起來像真的存在。最後另起一行，以「循環理由：」開頭，寫一句你此刻反覆播放這些歌的原因。這是播放清單，不是聊天：絕不要輸出訊息、台詞或想對任何人說的話;沒有「歌名 — 歌手」格式的行一律不合格。`,
  };
  ...
  const system = [
    ...
    // v87(p1):v82 讓快照繼承風格模組治好了安全咬,卻埋下格式走鐘——模組的
    // 「stay in character / you are texting」對「輸出資料快照」是反向指令,弱模型
    // 尤其容易聽錯邊(擁有者實案:整包不合格頻率飆高)。末端釘優先權,離輸出最近最大聲。
    '【格式優先權|最高】上方的全域指令與風格模組只提供內容尺度與世界觀框架。本次輸出是「資料快照」——不是對話、不是訊息、不是角色扮演場景;輸出的格式與內容形態以【任務】為唯一準則,凌駕任何角色扮演、訊息風格或敘事描寫的指示。',
  ].join('\n\n');
